...
const Header = () => {
    ...
    const [currentHeaderClass, setHeaderClass] = useState(isHome ? classNames("header-container", "transparent") : "header-container");
    const [headerIcons, setHeaderIcons] = useState(isHome ? true : false);
    const [sideNavbar, setSideNavbar] = useState({toggle: false, class:"header__menu--side-navbar"});
    ...
    const sideNavbarToggle = () => {
        if (!sideNavbar.toggle) {
            setSideNavbar({toggle:!sideNavbar.toggle, class: classNames("header__menu--side-navbar", "active")} );
        } else {
    ...
    return(    
        <header className={currentHeaderClass}>
            ...
            <Link to='/' className="header__logo">
                <img src={headerIcons ? headerElPortalWhiteIcon : headerElPortalBlackIcon} alt="El Portal Logo" />
            </Link>

            <Link to='#' className="menu__button--open" onClick={()=>sideNavbarToggle()}>
                <span className="button--open" style={{backgroundImage: `url(${headerIcons ? hamButtonWhite : hamButtonBlack})`}}></span>
            </Link>
